...
Troubleshooting: Once the installation on your host device is complete, verify that the agent is running by executing the following command (replace <your-org-name> with your organization’s name as found in the zip file name):
`docker logs -f agent-agent_<your-device-name>`
This command will display the logs of the running agent, confirming that is has been successfully installed or providing useful troubleshooting info otherwise.
